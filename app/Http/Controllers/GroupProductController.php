...
class GroupProductController extends Controller {
    ...
    public function datatable(Request $request)
    {
        ...
        $total_data = ProductGroup::count();

        $query_data = ProductGroup::where(function($query) use ($search, $request) {
                if($search) {
        ...
        $total_filtered = ProductGroup::where(function($query) use ($search, $request) {
                if($search) {
                    $query->where(function($query) use ($search) {
                        $query->where('name', 'like', "%$search%")
                            ->orWhereHas('updatedBy', function($query) use ($search) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if($validation->fails()) {
            ...
            $query = ProductGroup::find($id)->update([
                'updated_by' => session('id'),
                'name'       => $request->name,
                'status'     => $request->status
            ]);
    ...
    {
        $query = ProductGroup::find($request->id)->update(['status' => $request->status]);
        if($query) {
            activity('product group')
                ->performedOn(new ProductGroup())
                ->causedBy(session('id'))
    ...
    {
        $query = ProductGroup::destroy($request->id);
        if($query) {
            activity('product group')
                ->performedOn(new ProductGroup())
                ->causedBy(session('id'))
