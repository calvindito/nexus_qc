...
class ProcessController extends Controller {
    ...
    public function datatable(Request $request)
    {
        ...
        $total_data = Process::count();

        $query_data = Process::where(function($query) use ($search, $request) {
                if($search) {
        ...
        $total_filtered = Process::where(function($query) use ($search, $request) {
                if($search) {
                    $query->where(function($query) use ($search) {
                        $query->where('name', 'like', "%$search%")
                            ->orWhereHas('updatedBy', function($query) use ($search) {
    ...
    public function create(Request $request)
    {
        $validation = Validator::make($request->all(), [
            ...
            'name.required'   => 'Process cannot be empty.',
    ...
    {
        $query = Process::find($request->id)->update(['status' => $request->status]);
        if($query) {
            activity('process')
                ->performedOn(new Process())
                ->causedBy(session('id'))
    ...
    {
        $query = Process::destroy($request->id);
        if($query) {
            activity('process')
                ->performedOn(new Process())
                ->causedBy(session('id'))
